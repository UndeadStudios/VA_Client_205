...
public class RSFont extends Rasterizer2D {
	...
	private void drawCharacter(byte charPixels[], int x, int y, int width, int height, int color) {
		...
		if (x < Rasterizer2D.topX) {
			int offsetX = Rasterizer2D.topX - x;
			width -= offsetX;
			x = Rasterizer2D.topX;
			pixel += offsetX;
			offset += offsetX;
	...
	public void drawShadowedString(String string, int x, int y, int color, boolean shadow) {
		...
		if (strikeThrough) {
			Rasterizer2D.drawHorizontalLine(lineX, y + (int) ((double) baseCharacterHeight * 0.69999999999999996D), x - lineX, 0x800000);
		}
	}
	...
	public RSFont(boolean TypeFont, String s, StreamLoader archive) {
		...
		stream_1.currentPosition = stream.readUnsignedWord() + 4;
	...
	public void drawCharacter(int character, int i_35_, int i_36_, int i_37_, int i_38_, int i_39_, boolean bool) {
		...
		if (i_37_ > 0 && i_38_ > 0) {
			createCharacterPixels(Rasterizer2D.pixels, fontPixels[character], i_39_, i_43_, i_40_, i_37_, i_38_, i_41_, i_42_);
